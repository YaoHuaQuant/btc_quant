...
- 策略制定
    - 数据结构
      - [x] 虚拟订单 VirtualOrder
      - [x] 虚拟订单集合 VirtualOrderArrayInterface
      - [x] 虚拟订单簿 VirtualOrderBookInterface
      - [x] 订单调度器：虚拟订单 -> 实际订单 映射
        - 虚拟订单变化时 实际订单需要相应发生变化
      - [x] 策略 StrategyInterface
        - [x] only maker only long 做多波动率 中高频量化策略
          - 只做多策略。
          - 下跌时部分资金被套，需要面临一定的回撤。
          - 震荡时做T，高频小幅度低收益，并且生成高收益订单。
          - 上涨时令高收益订单获益，低频大幅度大收益。
          - （低风险，高回撤）在牛市时，假设价格一定会不断创新高，因此被套住的资金总是会解套。
        - [ ] only maker only short 做多波动率 中高频量化策略
          - 只做空策略
          - （高风险）在牛市时，如果价格创新高，很难跌回上一次新高处，被套住的资金可能无法解套。
        - [ ] only maker long short 做多波动率 中高频量化策略
          - 多空对冲策略
          - 在高位区只做空，低位区只做多，中间价格同时做空和做多。
          - 震荡时做T，高频小幅度低收益，同时生成了高收益的多空订单。
          - 向下突破时，高收益多单被套，高收益空单获利了结，低频大幅度大收益。
          - 向上突破时，高收益空单被套，高收益多单获利了结，低频大幅度大收益。
          - （中等风险，低回撤）必定会有回撤，并且空单可能在创新高时无法解套，但是回撤比例较只做多策略更低。
      - 策略
        - 挂单价格策略：由VirtualOrderList实现
          - [ ] (deprecated)Bubble策略：每个close_price只存在一个挂单，当有新的close_order时，将旧挂单的close_price上移。
            - 存在问题：
              1. 当价格震荡时间过长时，close_price容易超过上限。
              2. 会有大量close_order被套住，导致现金余额被击穿，在更低的价位无法进行买入操作。
          - [x] PriceOrderGroup策略: 同一个open_price的order组成一个group，并在一定close_price区间内进行挂单。
        - 挂单量策略：由StrategyInterface实现
          - [x] 现金等额分割策略：将总量现金资产进行等额分割，每个order的quantity恒定。
            - order_quantity = self.bet_cash_size = self.get_cash() / self.CASH_SLOT_NUM
            - 存在缺陷：
              - 如果分割粒度过粗，震荡下跌行情下容易出现现金余额被击穿的情况。
              - 如果分割粒度过细，无法有效利用所有资金。
          - [ ] 分价格区间等额分割策略：
            - 将整个开仓价格区间划分为多个价格梯度，限制每个梯度价格时的剩余现金量，若剩余现金不足，则不继续进行开仓操作。
            - [ ] 价格 -> 剩余现金量映射 策略
              1. 均匀分布（线性分布）: 剩余现金量 = 价格 * 系数A + 系数B。
              2. 非线性分布（适用于杠杆）：做多开仓价越高，提供的现金量增量越大。
        - [ ] 杠杆策略
          - [x] 动态杠杆
          - [x] 保证金管理
            - 开仓时借入
